...
use WPXCronManagerLight\PureCSSSwitch\Html\HtmlTagSwitchButton;
...
{
  // Columns
  public const COLUMN_HOOK_NAME = 'hook_name';
  public const COLUMN_SCHEDULE = 'schedule';
  public const COLUMN_SIGNATURE = 'signature';
  public const COLUMN_NEXT_RUN = 'next_run';
  public const COLUMN_COUNT_DOWN = 'count_down';
  public const COLUMN_EXECUTE = 'execute';
  ...
  {
    return sprintf('<input type="checkbox" name="cron[]" value="%s" />', $item['hook_name']);
  }
  ...
  public function column_count_down($item)
  {
    ...
    if ('on' == $item[self::COLUMN_STATUS]) {
      ...
      if (!empty($time)) {
        $prepost = $time > time() ? __('in') : __('late');

        $wait = __('wait...');

        return sprintf(
          '<span data-prepost="%s" data-cron="%s" class="wpxcm-ui-countdown" data-time="%s">%s</span>',
          $prepost,
          $item[self::COLUMN_HOOK_NAME],
          ($time - time()) * 1000,
          $wait
        );
      }
  ...
  public function column_status($item)
  {
    ...
    $class = 'on' == $item[self::COLUMN_STATUS] ? 'wpxcm-button-disable' : 'wpxcm-button-enable';

    $button = HtmlTagSwitchButton::name($hook_name)
      ->data('hook_name', $hook_name)
      ->checked('on' == $item[self::COLUMN_STATUS])
      ->class($class);
  ...
  public function prepare_items()
  {
    ...
    $per_page = $this->get_items_per_page('wp_cron_manager_per_page', 5);
    ...
    foreach ($crons as $timestamp => $cronhooks) {
      foreach ((array) $cronhooks as $hook => $events) {
        foreach ((array) $events as $sig => $event) {
          // Filter $schedule
          if (!empty($status) && !in_array($status, ['all', self::STATUS_DISABLED, self::STATUS_ENABLED])) {
  ...
  private function elapsedString($timestamp, $hide_empty = true, $to = 0, $separator = ', ')
  {
    ...
    $useful = [
      'y' => [__('Year'), __('Years')],
      'm' => [__('Month'), __('Months')],
      'd' => [__('Day'), __('Days')],
      'h' => [__('Hour'), __('Hours')],
      'i' => [__('Minute'), __('Minutes')],
      's' => [__('Second'), __('Seconds')],
    ];
